...
// posts.go resolves every mention typed into a post through this path, so a
// handle whose webfinger fails once — a peer that was down for a minute —
// stops resolving in posts from then on, with no way to retry short of
